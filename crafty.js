...
Crafty.c("Supportable", {
    ...
    _detectGroundTick: function() {
        ...
        var area = this._cbr || this._mbr || this,
            pos = this.__pos;
            pos._x = area._x;
            pos._y = area._y + 1; // Increase by 1 to make sure map.search() finds the floor
            pos._w = area._w;
            pos._h = area._h;
        ...
        if (ground) {
            ...
            if (!(ground.__c[groundComp] && overlap(garea, pos)) &&
                !(hasMap && 'map' in ground && this._SAT(this.map, ground.map))) {
        ...
        if (!ground) {
            var hit = false, obj, oarea,
                results = Crafty.map.search(pos, false), i = 0, l = results.length;
            for (; i < l; ++i) {
                obj = results[i];
                oarea = obj._cbr || obj._mbr || obj;

                // check for an intersection with the player
                if (obj !== this && obj.__c[groundComp] && overlap(oarea, pos)) {
                    if (hasMap && 'map' in obj && !this._SAT(this.map, obj.map))
                        continue;
                    hit = obj;
                    break;
                }
